...
public class FavoriteListController : ControllerBase
{
    ...
    public async Task<IActionResult> AddToFavorites([FromBody] FavoriteListDTO favoriteData)
    {
        ...
        var userIdClaim = User.FindFirst("StudentId") ?? User.FindFirst("DormitoryOwnerId") ?? User.FindFirst("AdministratorId");
        if (userIdClaim == null)
        {
    ...
    public async Task<IActionResult> RemoveFromFavorites([FromBody] RemoveFavoriteRequest model)
    {
        if (!int.TryParse(User.FindFirstValue("StudentId"), out int studentId))
        {
            _logger.LogWarning("StudentId claim not found in token.");
            return Unauthorized();
        }

        int? favoriteListId = await _favoriteListService.GetFavoriteListIdAsync(studentId, "Student");
        if (!favoriteListId.HasValue)
        {
            _logger.LogWarning("Favorite list not found for student ID {StudentId}", studentId);
            return NotFound();
        }
